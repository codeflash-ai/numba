...
class PassTimingsCollection(Sequence):
    ...
    def get_total_time(self):
        ...
        if records:
            # Use generator expression directly and avoid redundant attribute lookups per iteration
            get_total_time = type(records[0].timings).get_total_time
            return sum(get_total_time(r.timings) for r in records)
        else:
            return None
