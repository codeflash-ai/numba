...
def _adjust_timings(records):
    ...
    # Precompute totals for each time type once
    user_total = total_rec.user_time
    system_total = total_rec.system_time
    user_system_total = total_rec.user_system_time
    wall_total = total_rec.wall_time

    # Inline adjustment into a single tight loop, minimizing dict creation
    def adjust(rec):
        """Compute percent x total_time = adjusted"""
        # Fast path for namedtuple construction, avoiding full dict copies
        return PassTimingRecord(
            user_time=user_total * rec.user_percent * 0.01,
            user_percent=rec.user_percent,
            system_time=system_total * rec.system_percent * 0.01,
            system_percent=rec.system_percent,
            user_system_time=user_system_total * rec.user_system_percent * 0.01,
            user_system_percent=rec.user_system_percent,
            wall_time=wall_total * rec.wall_percent * 0.01,
            wall_percent=rec.wall_percent,
